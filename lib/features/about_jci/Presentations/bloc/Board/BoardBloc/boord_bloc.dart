...
class BoordBloc extends Bloc<BoordEvent, BoordState> {
  ...
  BoordBloc(
      this.getBoardByYearUseCase,
      this.addBoardUseCase,
      this.removeBoardUseCase,
      this.removeMemberBoardUseCase,
      this.addMemberBoardUseCase)
      : super(BoordInitial()) {
  ...
  void _onRemoveMemberBoard(
      RemoveMemberEvent event, Emitter<BoordState> emit) async {
    try {
  ...
  void onFetchBoardYearsEvent(
      FetchBoardYearsEvent event, Emitter<BoordState> emit) async {
    try {
      emit(const BoordState(
  ...
  Future<BoordState> _mapGetBoardOrFailure(
      Either<Failure, BoardYear> result, Emitter<BoordState> emit) {
    return result.fold(
      (l) async {
      ...
      (r) async {
        ...
        return state.copyWith(
            boardYears: [r], state: BoardStatus.Loaded, Listlength: list);
      },
    );
  }

  void _mapAddBoardOrFailure(
    Either<Failure, Unit> result,
    Emitter<BoordState> emit,
  ) {
    result.fold((l) {
      emit(BoordState(
          state: BoardStatus.Error, message: mapFailureToMessage(l)));
    }, (r) {
      log("'messagesssssss'");

      emit(BoordState(
          state: BoardStatus.Changed,
          message: 'Added',
          boardYears: state.boardYears));
    });
  }

  void _mapRemoveBoardOrFailure(
      Either<Failure, Unit> result, Emitter<BoordState> emit, String year) {
    result.fold((l) {
      emit(BoordState(
          state: BoardStatus.Error, message: mapFailureToMessage(l)));
    }, (r) {
      ...
      emit(BoordState(
          state: BoardStatus.Removed,
          message: 'Removed Successfully',
          boardYears: list));
    });
  }

  void _mapAddMemberBoardOrFailure(
      Either<Failure, Unit> result, Emitter<BoordState> emit) {
    result.fold((l) {
      emit(BoordState(
          state: BoardStatus.Error, message: mapFailureToMessage(l)));
    }, (r) {
  ...
  void _mapRemoveMemberBoardOrFailure(
      Either<Failure, Unit> result, Emitter<BoordState> emit) {
    result.fold((l) {
      emit(BoordState(
          state: BoardStatus.Error, message: mapFailureToMessage(l)));

...
class Dialogs {
  ...
  static void showPosAction(
      BuildContext context, Post post, TextEditingController SearchController) {
    showModalBottomSheet(
      isScrollControlled: true,
      useSafeArea: true,
      showDragHandle: true,
  ...
  static void showYearSelectionDialog(BuildContext context) {
    List<String> yearsList =
        List.generate(30, (index) => (2022 + index).toString());
    showDialog(
      context: context,
      builder: (BuildContext context) {
  ...
  static void showYearPresidentsSelectionDialog(BuildContext context) {
    List<String> yearsList = List.generate(DateTime.now().year + 4 - 1983,
        (index) => (1983 + index).toString()).reversed.toList();
    showDialog(
      context: context,
      builder: (BuildContext context) {
  ...
  static void showDelete(
      BuildContext context, String year, TypeDelete typeDelete, String roleid) {
    showDialog(
        context: context,
        builder: (BuildContext context) {
          return BoardComponents.alertDialogDelete(
              year,
              roleid,
              context,
              typeDelete,
              typeDelete == TypeDelete.Board
                  ? '${typeDelete.name} $year '
                  : typeDelete.name);
        });
  }

  static ShoUpdateAddPresident(
      BuildContext context, President? president, PresidentsAction action) {
    showModalBottomSheet(
      showDragHandle: true,
      isScrollControlled: true,
      useSafeArea: true,

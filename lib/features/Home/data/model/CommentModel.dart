...
class ActivityCommentModel extends ActivityComment{
  ActivityCommentModel({required super.Commentid,

    required super.activityId, required super.user,  required super.content, required super.Reactions, required super.replies, required super.createdAt});
  ...
  factory ActivityCommentModel.fromJson(Map<Object?, Object?> json) {
    ...
    return ActivityCommentModel(
      ...
      createdAt: mappedJson['createdAt']!=null?  DateTime.parse(mappedJson['createdAt'] as String):DateTime.now(),
      ...
      replies:mappedJson['replies']!=null?      (mappedJson['replies'] as Map).entries.map((entry){
      ...
      Reactions: mappedJson['reactions'] !=null? (mappedJson['reactions'] as Map).entries.map((reaction) {
        ...
        ReactionModel.fromJson(reactionData);}).toList():[],
  ...
  factory ActivityCommentModel.fromEntity(ActivityComment comment) {
    return ActivityCommentModel(
      ...
      replies: comment.replies.map((reply) => ReplyCommentModel.fromEntity(reply)).toList(),
      Reactions: comment.Reactions.map((reaction) => ReactionModel.fromEntity(reaction)).toList()
    );
  }
  // toJson
  ...
  Map<String, dynamic> toJson() {
    return {
      ...
      'replies': replies.map((reply) => ReplyCommentModel.fromEntity(reply).toJson()).toList(),
      'Reactions': Reactions.map((reaction) => ReactionModel.fromEntity(reaction).toJson()).toList(),

...
abstract class ParticipantsRemoteDataSource{
 ...
 Future<Unit> checkAbsence(String activityId, String memberId, String status) ;
  ...
  Future<List<ParticipantDetailsParam>> getAllParticipantsWithStatusOfActivity(String activityId);

  Future<Unit> sendReminder(String activityName, String activityBeginDate, String location) ;
}
class ParticipantsRemoteDataSourceImpl implements ParticipantsRemoteDataSource {
  ...
  ParticipantsRemoteDataSourceImpl({required this.logger, required this.firabaseFireStore});
  @override
  Future<Unit> checkAbsence(String activityId, String memberId, String status) async {
    ...
    try {
      ...
      final activityDoc = FirebaseFirestore.instance.collection('activities').doc(activityId);
      ...
      final participantQuery = await participantsCollection.where('memberId', isEqualTo: memberId).get();
      ...
      if (participantQuery.docs.isNotEmpty) {
        ...
        logger.i("Participant added and attendance set to $status for member $memberId");
...
///This function is used to get all the participants of an activity
  ...
  ///
  @override
  Future<List<ParticipantDetailsParam>> getAllParticipantsWithStatusOfActivity(String activityId)async {
    try {
      final participantsCollection = firabaseFireStore
          .collection('activities').doc(activityId).collection('Participants');
...
///This function is used to send a reminder to the participants of an activity
  ...
  ///
  @override
  Future<Unit> sendReminder(String ActivityName,String ActivityBeginDate,String location) async {
    try {
      final id=await const Store().getUserId();
...
///This function is used to update the attendance of multiple members in an activity
  ...
  ///
  @override
  Future<Unit> updateMembersAttendance({required String activityId, required List<ParticipantDetailsParam> members})async {
    try {
      // Reference to the activity document
      final activityDoc = FirebaseFirestore.instance.collection('activities').doc(activityId);

...
class ActivityDetailsComponent{
 ...
 static  Widget Description(MediaQueryData mediaQuery,Activity activitys) => Padding(
     ...
     Navigator.of(context).push(
       MaterialPageRoute(
         builder: (BuildContext context) {
           return  CreateUpdateActivityPage(id: 'id', activity: act, work: work, particpants: const [],);
         },
       ),
     );
 ...
 static   Widget ImageCard(mediaQuery,String image,BuildContext ctx) => image.isNotEmpty
      ? ClipRRect(
      ...
      child: InkWell(
        ...
        const   BoxDecoration(
            ...
            border: Border(bottom:  BorderSide(color: ColorsApp.textColorWhite)),
          ),
 ...
 static  Widget dots(BuildContext context, MediaQueryData mediaQuery,Activity activitys ) => Positioned(
      ...
      child: BlocListener<AddDeleteUpdateBloc, AddDeleteUpdateState>(
        ...
        child: GestureDetector(
          onTap: () {
            ActivityDetailsFunctions.  showGridModalBottomSheet(context, activitys);
          },
          child: Container(
            height:40,
            width: 40,
  ...
  static  Widget rowName(mediaQuery,BuildContext context,Activity activitys,activity act, int index) => Padding(
    ...
    child: Column(
          ...
          child: SingleChildScrollView(
            ...
            height: 30.h,
              child: ListView.separated(
                ...
                return Text("#${activitys.categorieId[index]}",style: PoppinsSemiBold(14.sp, PrimaryColor, TextDecoration.none),);
      ...
      //  PriceWidget(mediaQuery, activitys, context),
        ScrollingTextAnimation (address: activitys.ActivityAdress, name:  activitys.name,),
        Padding(
          padding: const EdgeInsets.all(8.0),
 ...
 static  Widget Back(mediaQuery, context) => Positioned(
      ...
      child: GestureDetector(
        ...
        child: Container(
          ...
          decoration:
          const BoxDecoration(color: BackWidgetColor,borderRadius: BorderRadius.all(Radius.circular(20))  ),
 ...
 static  Widget header(MediaQueryData mediaQuery, context,bool isMeeting) => SafeArea(
    child: Stack(
      ...
      children: [
        BlocBuilder<ActivityCubit, ActivityState>(
          builder: (context, state) {
            return Padding(
              padding:  EdgeInsets.symmetric(horizontal: mediaQuery.devicePixelRatio*12,vertical:40.h),
              child: Align(
                  alignment: AlignmentDirectional.center,
                  child: Text("${state.selectedActivity.name.tr(context)} Details",style: PoppinsSemiBold(mediaQuery.devicePixelRatio*6, isMeeting? ColorsApp.textColorBlack: ColorsApp.textColorWhite,TextDecoration.none),)),
            );
          },
        ),
      ],
   ...
   child: Row(
    ...
    builder: (context, state) {
      return Padding(
         ...
         FutureJoinButton(state, index, activitys, act, mediaQuery, mediaQuery.size.width / 2, mediaQuery.devicePixelRatio * 4.5),

       );
    },
   ),
       Pinnedbutton(onTap: (Activity ) {  }, isPinned: true, activity: activitys,)

     ],
   ),
 );

 static AnimatedSwitcher FutureJoinButton(AcivityFState state, int index, Activity activitys, activity act, MediaQueryData mediaQuery,double width,double textsize) {
   return AnimatedSwitcher(
       duration: const Duration(milliseconds: 1000), // Set the duration for the animation
       child: FutureBuilder(
         future: ActivityAction.checkifMemberExist(state.activitiesSearch[index].Participants),// Your asynchronous function that returns a Future
         builder: (context, snapshot) {
           if (snapshot.connectionState == ConnectionState.waiting || state.activityfetchState==ActivityFetchState.LoadingButton) {
             // Show a loading indicator while data is being fetched
             return ShimmerButton(width: width,height:  50);
 ...
 static  Widget kk(mediaQuery,Activity activitys)=>Padding(
   ...
   child: Row(
      ...
      children: [
        IconInfo(activitys.isOnline?Icons.online_prediction: Icons.place),
            ...
            SizedBox(
              ...
              child: InkWell(
                ...
                child: Padding(
                  ...
                  child: Text(
                    activitys.isOnline?activitys.googleMeetLink:
                    activitys.ActivityAdress,



                    style: PoppinsSemiBold(
                       17.sp, textColorBlack,activitys.isOnline?TextDecoration.underline:TextDecoration.none),
                  ),
                ),
              ),
            ),
 ...
 static  Widget infoCircle(mediaQuery,Activity activitys,BuildContext context) => SingleChildScrollView(
   child: Column(
     ...
     builder: (context, state) {
                ...
                child: Column(
                  ...
                  children: [
                    SizedBox(
                      width: mediaQuery.size.width/1.5,
                      child: Text(
                            DateFormat('EEEE dd MMM yyyy',state.locale==const Locale('en')?'en_US' : 'fr_FR').format(activitys.ActivityBeginDate).toUpperCase(),
      ...
      activityInfo(context: context, mediaQuery: mediaQuery, activity: activitys)
      ],
    ),
 );
  static Widget activityInfo({
    ...
    if (activity is EventModel) {
      ...
      imageUrl = activity.LeaderName.Images.isNotEmpty ? activity.LeaderName.Images[0] : null;
    } else if (activity is MeetingModel) {
      title = "Director Informations";
      name = "${activity.Director.firstName} ${activity.Director.lastName}";
      imageUrl = activity.Director.Images.isNotEmpty ? activity.Director.Images[0] : null;
                  ...
                  imageUrl != null?
                    Row(
                      ...
                      children: [
                        ...
                        AutoSizeText(
                          name,
                          style: PoppinsSemiBold(16.sp, textColorBlack, TextDecoration.none),
                        ),
                      ],
                    )
                  :
 ...
 static Widget IconInfo(IconData icon) {
   return SizedBox(
              ...
              child: Icon(icon, color: ColorsApp.ThirdColor,size:25,),

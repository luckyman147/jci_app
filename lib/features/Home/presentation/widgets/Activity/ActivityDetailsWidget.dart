...
class _ActivityDetailState extends State<ActivityDetail> {
  ...
  void initState() {
    ...
    if (widget.act != activity.Meetings) {
      context.read<ChangeStringBloc>().add(initImageEvent(image: widget.activitys.CoverImages));
    }
  }
  @override
    ...
    return SafeArea(
      child: Stack(
        ...
        children: [
          ...
          Padding(
            padding:widget.act!=activity.Meetings?  EdgeInsets.symmetric(vertical: mediaQuery.size.height / 2.5):EdgeInsets.symmetric(vertical:70.h),
            child: Column(
              mainAxisAlignment: MainAxisAlignment.start,
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                ActivityDetailsComponent.rowName(
                    mediaQuery, context, widget.activitys, widget.act, widget.index),
                NoImageCard(ColorsApp.BackWidgetColor,20),

                SectionViewer(act: widget.act,),
  ...
  BlocBuilder<ChangeStringBloc, ChangeStringState> BuildFirstPart(MediaQueryData mediaQuery) {
    return BlocBuilder<ChangeStringBloc, ChangeStringState>(
builder: (context, state) {
  return Stack(children: [
widget.act!=activity.Meetings?
ActivityDetailsComponent.ImageCard(mediaQuery, state.image,context)
          :
NoImageCard(ColorsApp.textColorWhite,100),
          ActivityDetailsComponent.Back(mediaQuery, context),
          ActivityDetailsComponent.header(mediaQuery, context,widget.act==activity.Meetings),
AddDots(widget.activitys, mediaQuery),

         widget.activitys.CoverImages.isNotEmpty?
         ImageListCard(images: widget.activitys.CoverImages .map( (e)=>e. toString()).toList(), ):const SizedBox(),

  ...
  IconButton buildIconButton(BuildContext context, int index) {
    return IconButton(
      onPressed: () {
        context
            .read<GetTaskBloc>()
            .add(DeleteChecklist(id, checkList[index]['id']));
  ...
  Checkbox buildCheckbox(int index, BuildContext context) {
    return Checkbox(
      ...
      onChanged: (bool? value) {
        final CheckInputFields checkInputFields = CheckInputFields(
            taskid: id,
            checkid: checkList[index]["id"],
            IsCompleted: value!,
            checklist: null,
            name: null,
            teamid: null);
        context
            .read<GetTaskBloc>()
            .add(UpdateChecklistStatus(checkInputFields));
        context.read<TaskVisibleBloc>().add(const ChangeIsUpdatedEvent(true));
      },
    );
  }
      ...
      suffixIcon: GestureDetector(
          onTap: () {
            if (controller.text.isEmpty) {
              ...
              context
                  .read<TaskVisibleBloc>()
                  .add(const ToggleTaskVisible(true));
              context.read<GetTaskBloc>().add(AddCheckList(checkInputFields));

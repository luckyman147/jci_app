...
Future<void> storeFcmToken(String token) async {
  try {
    ...
    if (currentUser != null) {
      ...
      String? storedUserId = await const Store().getUserId();

      if (storedUserId != null) {
        // If user ID is found in local storage, use it to store the token
        var userRef = FirebaseFirestore.instance.collection('users').doc(storedUserId);
        ...
        throw Exception('No authenticated user found and no user ID in local storage.');
      }
    }
  } catch (e) {

...
class PermissionsBloc extends Bloc<PermissionsEvent, PermissionsState>
    implements Strategy<PermissionsEvent, Emitter<PermissionsState>> {
  final LoadMasterPermissionsUseCase loadMasterPermissionsUseCase;

  PermissionsBloc(
    this.loadMasterPermissionsUseCase,
  ) : super(PermissionsLoadingState()) {
  ...
  @override
  Future<void> LoadPermissionsOfMaster(
      PermissionsEvent event, Emitter<PermissionsState> emit) async {
    try {
      // Cast the event to the specific type
      if (event is LoadPermissionOfMasterEvent) {
        if (state.permissions.isNotEmpty &&
            checkAllIdsExist(event.featuresId, state.permissions)) {
        ...
        emit(EithFailureorResponse<List<FeaturePermissions>>(
          ...
          (res) {
            return state.copyWith(
                permissions: res, type: TypePermissionsStatus.Loaded);
          },
        ));
      }
    } catch (e) {
  ...
  PermissionsState EithFailureorResponse<T>(
      Either<Failure, T> response, Function(T) onreturn) {
    return response.fold((failure) {
      return state.copyWith(
          type: TypePermissionsStatus.Error,

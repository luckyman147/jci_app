...
class PemissionsRepoImpl implements PermissionsRepository {
  ...
  final Handler<List<Feature>> featuresHandler;
  PemissionsRepoImpl(this.featuresHandler,
      {required this.remoteDataSources,
      required this.localDataSources,
      required this.unitHandler,
      required this.boolHandler,
      required this.featurePermissionsHandler});
  @override
  Future<Either<Failure, Unit>> addTemporaryPermissions(
      TempoPermissions tempoPermissions) async {
  ...
  Future<Either<Failure, bool>> checkPermission(
      CheckPermissionDtos checkPermissionDtos) async {
    final local =
        await localDataSources.checkLocaledPermission(checkPermissionDtos);
  ...
  Future<Either<Failure, List<FeaturePermissions>>> loadPermissionsOfUser(
      LoadPermissionsOfUser loadPermissionsOfUser) async {
    final local =
        await remoteDataSources.loadPermissionsOfUser(loadPermissionsOfUser);
  ...
  Future<Either<Failure, List<FeaturePermissions>>> loadPermissionsOfMaster(
    ...
    final local =
        await localDataSources.loadLocalPermissionsOfMaster(featureIds);
    ...
    return await featurePermissionsHandler.handle(onCall: () async {
      ...
      if (local.isNotEmpty) {
        ...
        if (missingFeatures.isNotEmpty) {
          final e =
              await remoteDataSources.AddMisingFeature(missingFeatures, local);
          Logger().w(e);
          return e;
        } else {
          return local;
      ...
      final remote =
          await remoteDataSources.loadPermissionsOfMaster(featureIds);
  ...
  @override
  Future<Either<Failure, List<Feature>>> loadAllFeatures() async {
    return await featuresHandler.handle(onCall: () async {
      return await remoteDataSources.fetchFeatures();
    }, onError: (error) {
      if (error is Exception) {
        return error.get_failure;
      }
